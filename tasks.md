# tasks.md — タスク管理

ステータス: `[ ]` 未着手 / `[~]` 進行中 / `[x]` 完了

---

## 完了済みタスク

- [x] リポジトリ初期セットアップ（エージェント定義、チャネル仕様、中古市場ルール）
- [x] 商品検索: キーホルダー型 GPS トラッカー（2026-04-08）
  - 成果物: reports/search_20260408_1046_v4.md
- [x] 商品検索: フィナステリド（2026-04-09）
  - 成果物: reports/search_20260409_0607_finasteride_v1.md
- [x] 商品検索: 子ども向け絵本（2026-04-10）
  - 成果物: reports/search_20260410_0717_picturebooks_v1.md
- [x] 商品検索: 水筒（2026-04-10）
  - 成果物: reports/search_20260410_0940_waterbottle_v1.md, search_20260410_1200_waterbottle_budget_v1.txt
- [x] 商品検索: スマートフォン（2026-04-11）
  - 成果物: reports/search_20260411_0216_smartphone_v1.md
- [x] 商品検索: スマートスピーカー（2026-04-11）
  - 成果物: reports/search_20260411_0837_smartspeaker_v1.md
- [x] 犬種比較レポート（柴犬/ボーダーコリー/日本スピッツ/コーギー/ハスキー）（2026-04-13）
  - 成果物: reports/dog_breed_comparison_20260413.md
- [x] 脱毛調査レポート（眉毛 + VIO 医療脱毛）（2026-04-17）
  - 成果物: reports/hair_removal_guide_20260417.md
- [x] リポジトリガバナンス整備（2026-04-18）
  - CLAUDE.md を軽量コアに再構成
  - docs/rules-search-product.md を新規作成（商品検索詳細ルール）
  - tasks.md を新規作成（本ファイル）
  - file_index.md を新規作成
  - main ブランチへ移行（git push origin HEAD:main）
- [x] 商品検索: ロキソニンSテープ 7cm・14cm 1枚単価最安値調査（2026-04-19）
  - 成果物: reports/search_20260419_0422_loxonin_tape_v1.md（最終）
  - Amazon + Yahoo!ショッピング 2チャネル調査（楽天はタイムアウトにより除外）
- [x] 商品検索: こどもが歯磨きしたくなる絵本（2026-04-19）
  - 成果物（最終）: reports/search_20260419_hamigaki_ehon_v1.md
  - 楽天+Yahoo!+Amazon 3チャネル調査（AmazonはASIN確認のみ）
  - TOP3: ノンタン はみがきはーみー / はぶらしくんです。/ こぐまのくうぴい すきすきはみがき
- [x] 商品検索: ロキソプロフェンNaテープ 代替品追加調査 → v2レポート生成（2026-04-19）
  - 成果物: reports/search_20260419_loxonin_tape_v2.md（最終）
  - 代替品で小サイズ¥42/枚・大サイズ¥57/枚を発見（v1比 -55〜70%）
  - TOP3全差替え済み
- [x] 商品検索: 室内用靴下（夏用・薄手・27cm以上）（2026-04-22）
  - 成果物（最終）: reports/search_20260422_room_socks_v1.md
  - 楽天+Yahoo! 2チャネル調査（Amazon はタイムアウト除外）
  - TOP3: HARUSAKU綿日本製 / 綿麻接触冷感5足apple / 竹繊維つるや
- [x] 商品検索: メンズ タートルネック ロングTシャツ（2026-04-26）
  - 成果物: reports/search_20260426_1500_mens_turtleneck_longt_v1.txt
  - Yahoo!ショッピング 単独チャネル調査
  - 8商品調査、在庫あり候補 5 点掲載
  - TOP3: N-MART(¥1,480・311件) / ARUGE(¥1,198・364件) / FLAG ON CREW(¥1,498・39件)
- [x] 商品検索: メンズ タートルネック/ハイネック ロングTシャツ 胸ポケット付き Amazon.co.jp 専属調査（2026-04-26）
  - 成果物: reports/search_20260426_1800_mens_turtleneck_amazon_v1.txt
  - Amazon.co.jp 単独チャネル調査（サクラチェッカー検証付き）
  - TOP3: アタックベース 850-15(¥1,430・合格) / Men's7 無地(¥1,990・注意) / Men's7 ボーダー(¥1,660・注意)
- [x] 商品検索: メンズ タートルネック/ハイネック/モックネック ロングTシャツ 胸ポケット付き Yahoo!ショッピング調査（2026-04-26）
  - 成果物: reports/search_20260426_1900_mens_turtleneck_pocket_v1.txt
  - Yahoo!ショッピング 単独チャネル調査（延べ18商品ページを WebFetch 確認）
  - 全条件適合品は2件のみ（市場に極めて少ない）
  - 確認済み: ARUGE(¥1,198・364件) / サンキ(¥878+送料・50件)
  - 在庫補充待ち参考: KRIFF MAYER(¥7,590・在庫なし）
- [x] 商品検索: メンズ タートルネック/ハイネック/モックネック ロングTシャツ 胸ポケット付き 最終版（2026-04-26）
  - 成果物（最終）: reports/search_20260426_turtleneck_pocket_v1.md
  - Yahoo!+Amazon+楽天 3チャネル調査、胸ポケット付き確認済み5件
  - TOP3: ARUGE(¥1,198・Yahoo!) / アタックベース850-15(¥1,430・Amazon・綿100%・サクラ合格) / GOODWEAR MOCKNECK(¥3,980・楽天・Made in USA)
- [x] 商品検索: スマートフォン接続対応 小型プロジェクター（格安・WiFiミラーリング対応）Amazon.co.jp 専属調査（2026-04-27）
  - 成果物: reports/search_20260427_1200_projector_smartphone_v1.txt
  - Amazon.co.jp 単独チャネル調査（サクラチェッカー検証付き）
  - カテゴリ全体でサクラ製品64%という危険カテゴリを調査
- [x] 商品検索: スマートフォン接続対応 小型プロジェクター（格安）最終版（2026-04-27）
  - 成果物（最終）: reports/search_20260427_projector_smartphone_v1.md
  - Yahoo!+楽天+Amazon 3チャネル調査（Amazon はサクラリスク高のため参考扱い）
  - TOP3: SOPPY z700024(¥9,580・Yahoo!・Android TV内蔵・4.48/292件) / Era-Pioneer YT100-E(¥6,480・Yahoo!・最安値) / スリス dppj1052(¥9,980・楽天・4.42/671件)
- [x] リサーチ: 楽天市場における偽レビュー・サクラレビューの実態と割合推計（2026-04-28）
  - 成果物: reports/research_20260428_rakuten_fake_reviews_v1.md
  - サクラチェッカー対応状況・ポイント制度変遷・DC8事件・ステマ規制後対応を調査
- [x] リサーチ: Yahoo!ショッピング偽レビュー実態・消費者庁ECサイト横断調査（2026-04-28）
  - 成果物: reports/research_20260428_fake_reviews_yahoo_ec.txt
  - サクラチェッカーYahoo!対応状況、LINEヤフー公式削除実績、消費者庁・FTC横断データ、ツール比較
- [x] リサーチ: ECサイト別 偽レビュー実態比較 最終版（2026-04-28）
  - 成果物（最終）: reports/research_20260428_fake_reviews_ec_comparison_v1.md
  - Amazon/楽天/Yahoo! 3サイト横断比較・サクラ判定軸・割合推計・ツール比較
  - 結論: サクラチェッカー合格Amazon商品を選ぶ戦略が最も合理的
- [x] 商品検索: スマートフォン接続対応プロジェクター サクラ合格品限定調査（2026-04-28）
  - 成果物: reports/search_20260428_1500_projector_sakura_v1.txt
  - Amazon.co.jp 専属・サクラチェッカー合格ランキング全41件確認
  - 結論: ¥20,000以下・WiFiミラーリング対応・サクラ合格品はゼロ（カテゴリ構造的問題）
  - 参考: FUNLOGY Projector(¥9,980・合格・WiFi非搭載) / JEJILL(¥31,999・注意・WiFi対応)
- [x] 商品検索: スマートフォン接続対応プロジェクター サクラ合格品 TOP3 v2（2026-04-28）
  - 成果物（最終）: reports/search_20260428_projector_sakura_v2.md
  - Amazon サクラ合格限定・¥20,000以下合格品ゼロのため予算拡大版
  - TOP3: Anker Nebula Astro(¥32,980・合格・424件) / Xiaomi L1(¥29,980・合格・63件) / FUNLOGY+FireTV(¥14,960・合格・間接接続)
- [x] 商品検索: メンズ靴下 グレー無地・27-29cm・ハーフ丈・口ゆったり 全5チャネル調査（2026-05-06）
  - 成果物: reports/search_20260506_1000_socks_grey_v1.txt
  - 楽天市場 2チャネル候補採用（Amazon/iHerb/オオサカ堂: 取扱なし、メルカリ: 全件⚠️で除外）
  - TOP2: lulu-mavis スクールソックス(¥1,400) / レモコロ 超幅広ゆるゆる(¥1,100)
- [x] 商品検索: UVカット・フェイスカバー付き・接触冷感パーカー Amazon.co.jp 専属調査（2026-05-06）
  - 成果物: reports/search_20260506_1200_uv_face_cover_parka_v1.txt
  - 10商品調査、在庫切れ2件・サクラ複数フラグ6件・条件未確認1件
  - TOP3: Dttrol B0C3W3SQWC(全フラグなし・価格不明) / ノーブランド B0DBP4T5JD(¥2,577・全フラグなし) / kimsoo B0D2NZ4TFQ(¥1,500・注意フラグ付き)
  - カテゴリ全体でサクラ疑い品が多く、3条件充足かつサクラ合格の完全版は未発見
- [x] 商品検索: UVカット・フェイスカバー付き・接触冷感パーカー 最終版（2026-05-06）
  - 成果物（最終）: reports/search_20260506_uv_face_cover_parka_v1.md
  - 楽天+Yahoo!+Amazon 3チャネル調査
  - TOP3: smart-shop(¥1,880~・楽天・2,115件・TV紹介) / えるえる(¥1,980・Yahoo!・4.44/194件・送料無料) / matsucame忍者パーカー(¥3,480~・楽天・Q-MAX0.3+・1,479件)
- [x] 商品検索: 犬のうんち臭対策ゴミ箱・消臭グッズ（ゴミ箱蓋・取り付け型消臭剤）Amazon.co.jp 専属調査（2026-05-17）
  - 成果物: reports/search_20260517_1000_pet_deodorizer_v1.txt
  - Amazon.co.jp 専属調査 / 8 ASIN 調査・サクラチェッカー検証
  - サクラ除外 4 件（ドギーマン/OFT/ペットリア/コモライフ）
  - TOP3: GEX デオケア消臭ペール犬用(B084ZYCPV9・¥2,300~・合格) / GEX デオケア貼る消臭剤(B09YCGJ1BT・¥300~/個) / デオワンダー ペット用(B0B1PS6FQ5・¥866/2個・合格)
- [x] 商品検索: 犬のうんち臭対策ゴミ箱・消臭グッズ 最終版（2026-05-17）
  - 成果物（最終）: reports/search_20260517_pet_deodorizer_v1.md
  - 楽天+Yahoo!+Amazon 3チャネル調査
  - TOP3: GEX デオケア消臭ペール犬用(楽天¥2,206~・Amazon合格・Wブロック) / GEX デオケア貼る消臭剤(Amazon¥300~/個・蓋内側貼り付け) / デオワンダー ペット用2個入(Amazon¥866・CLO2・3チャネル全取扱)
- [x] 商品検索: 屋外用・耐UV・耐候性の粘着テープ メルカリ専属調査（2026-05-18）
  - 成果物: reports/search_20260518_1400_uv_tape_mercari_v1.txt
  - メルカリ単独チャネル調査（Buyee経由在庫確認）
  - SOLD除外 7 件以上（延べ 30 件以上検証）
  - TOP3: m89885442137 アルミブチルテープ35cm×10m(¥7,300・新品) /
          m97313905218 補修ブチルテープ10cm×5m(¥1,998・新品) /
          m28905298659 不織布ブチルテープ10cm×5m 2年保証(¥3,489・新品・要確認)
- [x] 商品検索: 屋外用・耐UV・耐候性の粘着テープ 全5チャネル最終版（2026-05-18）
  - 成果物: reports/search_20260518_1400_uv_tape_final.txt
  - Amazon・楽天 候補採用（メルカリ: 耐UV明示なし全除外 / iHerb・オオサカ堂: 取扱なし）
  - TOP3: 因幡電工UVT-50-I(¥1,110・Amazon) / KISS STRONGER(¥737・楽天) / ゴリラテープClear(¥1,230〜・Amazon)
- [x] 商品検索: 屋外用・耐UV・耐候性の透明（クリア）粘着テープ Amazon.co.jp 専属調査（2026-05-18）
  - 成果物: reports/search_20260518_1800_uv_tape_clear_amazon_v1.txt
  - Amazon.co.jp 単独チャネル調査（サクラチェッカー検証付き）
  - 透明（クリア）＋耐UV明示＋屋外防水の厳格条件で調査
  - TOP3候補: KURE ゴリラテープ クリスタルクリア(B0DZNMYYML) / コニシ ストームガードクリヤー(B002YMOU8A) / シリコン防水テープ 耐紫外線(B09GNKCDY2)
- [x] 商品検索: Windows PC用 USB/USB-C 接続イヤホン（オンライン会議向け）Amazon.co.jp 専属調査（2026-05-18）
  - 成果物: reports/search_20260518_1000_usb_earphone_meeting_v1.txt
  - Amazon.co.jp 専属調査 / 8 ASIN 調査・サクラチェッカー検証
  - サクラ除外 2 件（UGREEN B08TQKNC62/ELECOM B099RMTHLL）
  - TOP3: ロジクール H390r(B079WKMMBM・¥3,709・合格) / ロジクール H340r(B079PTGNTW・¥3,300・合格) / Poly BW3325 USB-C(B0C6FPNWDK・¥12,999・注意)
- [x] 商品検索: Windows PC用 USB/USB-C 接続イヤホン・ヘッドセット 最終版（2026-05-18）
  - 成果物（最終）: reports/search_20260518_usb_earphone_meeting_v1.md
  - 楽天+Yahoo!+Amazon 3チャネル調査
  - TOP3: ロジクール H390r(Amazon¥3,709・サクラ合格・NCマイク・コントローラー) / ロジクール H340r(Amazon¥3,300・サクラ合格・4.9評価) / エレコム HS-HP07SUBK(楽天¥2,292~・国内メーカー・NCマイク)
- [x] 商品検索: メンズ靴下 グレー無地・27-29cm・ハーフ丈/クルー丈・口ゆったり Amazon.co.jp 専属調査（2026-05-18）
  - 成果物: reports/search_20260518_2100_socks_grey_amazon_v1.txt
- [x] 商品検索: メンズ靴下（グレー無地・27-29cm・ハーフ丈/クルー丈・口ゆったり）Yahoo!ショッピング新規開拓調査（2026-05-18）
  - 成果物: reports/search_20260518_2100_socks_grey_yahoo_v1.txt
  - Yahoo!ショッピング単独チャネル調査（延べ22回検索・16ページWebFetch確認）
  - 全条件充足1件: ソックスマルシェ A-29（¥640+送料・27-29cm・グレー無地・クルー丈・口ゴムゆったり明記）
  - 条件C要確認2件: HARUSAKU ライトグレー/ダークグレー jp-1（¥1,100・日本製・27-29cm）
  - 参考3件: レモコロ100-695/100-697（ハーフ丈・27or29cmどちらか非対応）、福助ふくらし白鳩（29cm非対応）
- [x] 商品検索: メンズ靴下 グレー無地・27-29cm・ハーフ丈・口ゆったり 改訂版 v2（2026-05-18）
  - 成果物（最終）: reports/search_20260518_2200_socks_grey_v2.md
  - 楽天+Yahoo!+Amazon 3チャネル調査（Yahoo!は前回未調査の新規開拓）
  - 前回1位(lulu-mavis)除外、追加候補5件発掘
  - TOP5: コベス5693+5694(¥528/足・楽天) / ソックスマルシェA-29(¥640/足・Yahoo!) / レモコロ00-694(¥1,100・楽天・前回2位) / レモコロ778413(¥700-800推定・楽天) / HARUSAKU jp-1(¥1,100・Yahoo!/楽天)
- [x] 商品検索: ノートPC（軽量・14インチ・Ryzen5/Core i5第12世代以降）Yahoo!ショッピング専属調査（2026-05-19）
  - 成果物: reports/search_20260519_1000_notebook_pc_yahoo_v1.txt
  - Yahoo!ショッピング 単独チャネル調査（延べ30件以上のページ確認）
  - 全必須条件充足（予算¥70,000以下含む）かつ在庫ありの候補: 0件
  - 準合格TOP3（予算超・他条件充足・在庫あり確認済み）:
    HP Pavilion Aero 13-bg(¥169,800・0.99kg・11.5h・Ryzen5 8640U・HP Directplus) /
    HP Pavilion Aero 13-bg Office付(¥194,800・0.99kg・HP Directplus) /
    ASUS VivoBook Go 14 E1404FA(¥93,280・1.38kg・11.5h・Ryzen5 7520U・ケーズデンキ)
- [x] 商品検索: ノートPC（軽量・14インチ・Ryzen5/Core i5第12世代以降）Amazon.co.jp 専属調査（2026-05-19）
  - 成果物: reports/search_20260519_1000_notebook_pc_amazon_v1.txt
  - Amazon.co.jp 専属調査 / 15 ASIN以上調査・サクラチェッカー検証
  - ¥70,000以下で全条件充足モデルは確認できず（市場構造的問題）
  - 準合格TOP3: ASUS Vivobook Go 14(B0C58Y6QL8・¥84,800・サクラ合格) / IdeaPad Slim3 Gen8 14(B0DBHLDP7D・¥79,480相当・要確認) / IdeaPad Slim3 Ryzen5 8640HS(B0GGB3C5QV・¥88,800)
  - サクラ除外: B0F4JWM8WR(i5-13420H・60%危険) / B0CJ4S11CT(51%注意・在庫切れ可能性)
- [x] 商品検索: ノートPC（軽量・14インチ・Ryzen5）楽天市場専属調査（2026-05-19）
  - 成果物: reports/search_20260519_1000_notebook_pc_rakuten_v1.txt
  - 楽天市場専属調査（40クエリ、WebFetch約10件実行）
  - 準合格1件: HP 14 限定モデル 三信テック(¥69,900・Ryzen5 7530U・1.39kg・10h・価格は参照時点)
  - 要確認候補: IdeaPad Slim 3 Gen 8 BJY通販(価格不明) / Lenovo公式直販(P10倍・価格要確認)
- [x] 商品検索: ノートPC（カフェ作業用・軽量・¥70,000以内）最終版（2026-05-19）
  - 成果物（最終）: reports/search_20260519_notebook_pc_v1.md
  - Amazon+楽天+Yahoo! 3チャネル横断調査
  - ¥70,000以下で全条件充足の新品は3チャネル合わせてもほぼなし（市場構造的問題）
  - TOP3: IdeaPad Slim 3 Gen 8 Ryzen5 7530U(楽天/Amazon・1.37kg・13.7h・セール時¥63,910実績) / HP 14 三信テック楽天(参照¥69,900・要確認) / ASUS VivoBook Go 14(Amazon¥84,800・サクラ合格)

- [x] リサーチ: モビコール・類似品 処方なし入手方法 調査レポート（2026-05-31）
  - 成果物: reports/research_20260531_movicol_otc_alternatives_v1.md
  - ①ドラッグストア代替品（酸化マグネシウム系）/ ②オンライン診療 / ③iHerb MiraLax個人輸入 / ④空詩堂個人輸入代行 の4ルートを整理

---

## 進行中

（なし）

---

## バックログ

- [ ] 商品検索ルールの定期レビュー（失敗パターン更新時）
- [ ] エージェント定義のバージョン管理（.claude/agents/ 以下）

...
### 3.1 比較・ランキング・スコアリングレポートの絶対ルール（再発防止）
詳細は `docs/rules-search-product.md`（失敗パターン6・第0.5章・第4章）参照。

**★着手時トリガー（最優先・機械的に発火させる）**：依頼文に
「比較／ランキング／おすすめ／TOP◯／ベスト／一番（良い・安い）／コスパ最強／
◯点満点で評価／順位／どれがいい／選ぶなら」のいずれかが含まれたら、**最初の応答で**
(a) 「これは比較・ランキング系タスク」と宣言し、(b) スコアリングに着手せず
**先にブランド地図（候補一覧）を作成してユーザーに提示**する。
「ルールを思い出す」のではなく「この語に反応する」こと。失敗パターン6は本ルール
追加の28時間後に再発した（8製品で確定→指摘後12製品→TOP1入替）。文章追加では
防げないため、語トリガー＋スコアリング前の成果物ゲートで止める。

要点：

1. **市場全体スキャン必須（最重要）**：一部の偏ったブランド・商品だけで
   ランキングを確定しない。カテゴリの主要ブランド（国内大手／海外大手／
   EC専業・新興まで）を体系的に列挙し、第三者の比較記事で漏れをクロス
   チェックしてから着手する。除外製品も理由つきで全件レポートに明記する。
   過去にこの網羅性欠如で1位が入れ替わる大失敗をしている。**二度と繰り返さない**。
2. **スコア比較表の商品名はリンク必須**：商品名セルは購入リンク
   `[商品名](URL)` にする。優先順位は Amazon＞楽天＞メルカリ＞（無ければ）公式。
   在庫のある個別商品ページを優先する。
3. **変更履歴をレポート本文に書かない**：更新ごとの「○○からの変更」「改訂点」
   「vN→vN+1 変化」等の差分説明は本文に残さない。最終版の結論だけを記載する。
   変更の記録が要る場合は index/file_index の1行説明か commit message に留める。
4. **スコアリング前に在庫・条件確認を必ず完了させる（ルールD）**：在庫確認なし・
   条件充足確認なしの候補をスコアリング表に含めてはならない。在庫確認は
   「否定情報なし」では不十分で2ソース以上の肯定確認が必要。「商品名にキーワードが
   含まれる」は条件充足確認として不十分（製品ページでの一次情報確認が必要）。

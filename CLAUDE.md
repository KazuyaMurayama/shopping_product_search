...
### レポート序盤の集計表に購入リンク必須（恒久ルール）

レポート序盤（`## 0. 結論サマリー` / TOP5 / スコアリング表 / 総合ランキング等）に掲載するすべての商品一覧表は、
**必ず最右列に「購入リンク」列を設け `[表示名](URL)` 形式のリンクを記載する**。

- ユーザーがレポート冒頭だけを見て、気に入った商品をワンクリックで購入ページへ飛べる状態にする
- リンクが取得できない場合は `—` を入れ、理由を備考に明記する
- この要件は今後作成するすべての調査・商品検索レポートに適用する

...
楽天は EUC-JP の影響でスニペットが古い可能性がある。候補特定後、
全候補について以下を必ず実施：

1. **否定情報プローブ**：商品名 + `"在庫切れ"` / `"SOLD OUT"` /
   `"販売終了"` / `"取扱終了"` で追加検索
2. **二次ソース確認**：kakaku.com / 商品公式ページ / プレスリリース
   で現行販売確認
3. **在庫情報がスニペット内の `sold-out` / `quantity: 0` にないかチェック**
   （v2 で Tile Slim Gen3 acstore の sold-out を発見できた例）
